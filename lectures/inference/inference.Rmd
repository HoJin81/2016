...
Now that we have described sampling models, now we can describe properties of the sum of draws. The first important concept to learn is the _expected value_. The random variable will vary around this expected value. The _standard error_ gives us an idea of how the size of the variation around the expected value.
...
- Betting on black: 1*(9/19)-1*(10/19) = -1/19
- Betting on 7: 1*(1/38) - 1(37/38) = -36/38
...
[HJS] law of independence가 large N보다 우선한다.

### Estimates

Beacuse $S/25$ (= average of the poll) is $p$ plus or minus some chance error we use it as our _estimate_ of $p$. We usually put hats on top of our estimates: $\hat{p}$. We now now that when $N$ is large $\hat{p} \approx p$.
...
cat("Our estimate of the percent of decomrats is",p_hat,
    "plus or minus", round( sqrt(p_hat*(1-p_hat)/25), 2))
# This estimation is not very precise. We can use statistical method, below.
```
...
```{r}
# error (=p_hat-p) is a random variable
N <- 625
B <- 10^4
error <- replicate(B, {
  X <- sample(x, N, replace=T)
...
hist(error) # Expectation of error is '0'.
=======
...
qqnorm(error)
qqline(error,col=2)
# In this case, normal approximation works well. We can just show whether CLT works, using Q-Q plot.
```
...
mean(abs(error) > 0.02)
##to
pnorm(-1) + (1-pnorm(1)) # [HJS] Proportion of less than -1 SD from the mean and greater than 1 SD from the mean
=======
...
mean(error)
sqrt( mean(error^2))
sigma/sqrt(N) # this is standard error
hist(error) # to check it is normal
qqnorm(error) # to check it is normal
```
...
```{r}
## if mean(X)-mu is less than 0 ??
## if x is 0, the error is: 
0-mu
## [HJS] That is, if my earning is less than '0', the casino losese money.
```
...
```{r}
## we want to make "(sum/N) > 0"
## (sum/N)-mu > 0-mu == error > 0-mu 
## --> divide the above by sigma/sqrt(N)
## --> then, left side becomes normal distribution
## then, we can just do '1-pnorm((0-mu)/(sigma/sqrt(N)))'
1 - pnorm(-sqrt(N)*mu/sigma) # this is approximation. this should be 0.1459203... but mine is just 0.2222.....
mean(error > -mu) # this is precise
pnorm(0, mean=(-1/19), sd=2*sqrt(9/10*10/19), lower.tail=F) # <- wrong
# N gets larger, error gets smaller
qnorm(0.999)
```
...
hist(y) ## [HJS] normal distribution
```
...
3. What is the probability that our estimate is within one inch of the population average? http://goo.gl/forms/zIiqhGuVCl

```{r}
## [HJS]
## we want to know Xbar-mu < 1 inch
## --> devide both sides by sigma/sqrt(N)
## --> put the left side ( 1/(sigma/sqrt(N)) = sqrt(N)/sigam ) in pnorm...
## By looking at "sqrt(N)/sigam", we can estimate how big the probability would be..
pnorm(sqrt(N)*1/sigma) - pnorm(-sqrt(N)*1/sigma)
```
...
```{r}
# Here, ci is random variable.
ci <- p_hat + c(-1,1)*qnorm(0.975)*sqrt(p_hat*(1-p_hat))/sqrt(N)
p 
ci[1] <= p & ci[2] >= p # Cheking if p is within CI.
```
...
tab  <- replicate(B,{
  ...
  ci <- p_hat + c(-1,1)*qnorm(0.975)*sqrt(p_hat*(1-p_hat))/sqrt(N)
  hit <- ci[1] <= p  & ci[2] >= p
  c(p_hat,ci,hit) # Retruning
})
# [HJS] If we change qnorm(0.995), it's 99% confidence...
...
# [HJS] There are 7 "Miss". If we replicate 1000, then there would be less "Miss".
```
...
4. No use a Monte Carlo simulation to re run this experiment (keeping `prob_heads` fixed) 10000 times. Do we confirm that we cover 95% of the time? http://goo.gl/forms/GqHUcZkSDM
